...
export function LiveChart() {
  ...
  useEffect(() => {
    ...
    const sizeTo = () => {
      ...
      if (w === lastW && h === lastH && ctx) return { w, h, dpr };
      lastW = w;
      lastH = h;
    ...
    const draw = () => {
      try {
        ...
        const crowd = s.crowd;
        const liveOpenI = s.liveOpenI;
        const strike = s.phase === "live" ? s.startPx : null;
    ...
    <div className="relative h-full w-full min-h-0" ref={wrapRef}>
      {!ready && (
        <div className="absolute inset-0 z-[1] flex flex-col justify-between p-4">
          ...
          <SkeletonRows n={8} className="h-2 w-full" />
          <Skeleton className="ml-auto h-6 w-24 rounded-full" />
        </div>
      )}
      <canvas className="block h-full w-full" ref={canvasRef} />
      <div className="absolute right-2 bottom-3 z-10 flex flex-col overflow-hidden rounded-full border border-white/15 bg-black/55 max-md:right-1 max-md:bottom-2">
        <button
          type="button"
